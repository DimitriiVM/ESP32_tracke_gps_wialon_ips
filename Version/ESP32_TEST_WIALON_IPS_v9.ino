...
//Для  сокращенного пакета, с 9 версии только полный пакет
char lat1 [13] = "00.01";
char lat2 [2] = "N";
char lon1 [13] = "00.01";
char lon2 [2] = "E";
...
char adc [7] = "0";
char ibutton [7] = "0";
char params[256] = "SOS:1:0";
/*
Набор дополнительных параметров через запятую. Каждый параметр представляет собой конструкцию NAME:TYPE:VALUE
NAME – произвольная строка, длиной не более 15 байт
TYPE – тип параметра, 1 – int, 2 – double, 3 – string
...
bool StatusOUT0 = false; //Храним состояние для открытия выхода по минусу
bool StatusOUT1 = false; //Храним состояние для открытия выхода по минусу
bool StatusOUT2 = false; //Храним состояние для открытия выхода по минусу
bool StatusOUT3 = false; //Храним состояние для открытия выхода по минусу

void setup() {
  Serial.begin(115200);
  Serial.setTimeout(50);
  pinMode(OUT0, OUTPUT);//Иницилизация 0 выхода
  pinMode(OUT1, OUTPUT);//Иницилизация 1 выхода
  digitalWrite(OUT0, HIGH); //Выключаем его сразу
  digitalWrite(OUT1, HIGH); //Выключаем его сразу
...
void loop() {
  ...
  if(millis() - Timer2 >= 10000){//Делаем пинг каждые 2 секунды
    Timer2 = millis();
  ...
  if(millis()-Timer5 >= 30000){//Отправка пакетов сокрашенных SD
    Timer5 = millis();
  ...
  if(millis() - Timer8 >= 100){//Мы ищем команды и текст по Serial
    Timer8 = millis();
...
void AnswerPackServer(){
  ...
  while(millis()-timer <= 100){//Ждем ответ с авторизацией с таймаутом  100
   ...
   if(i > 1){
    ...
    char SendPackM[64] = "#M#";
    if(strncmp(AnswerPackServer, "OUT0=", 5) == 0)
    {
      if(AnswerPackServer[5] == '0')
      {
        StatusOUT0 = false;
      }
      if(AnswerPackServer[5] == '1')
      {
        StatusOUT0 = true;
      }
      client.printf("OUT0=%d\r\n", StatusOUT0);
    }
    else if(strncmp(AnswerPackServer, "OUT1=", 5) == 0)
    {
      if(AnswerPackServer[5] == '0')
      {
        StatusOUT1 = false;
      ...
      client.printf("OUT1=%d\r\n", StatusOUT1);
    }
    else if(strncmp(AnswerPackServer, "OUT2=", 5) == 0)
    {
      if(AnswerPackServer[5] == '0')
      {
        StatusOUT2 = false;
      ...
      client.printf("OUT2=%d\r\n", StatusOUT2);
    }
    else if(strncmp(AnswerPackServer, "OUT3=", 5) == 0)
    {
      if(AnswerPackServer[5] == '0')
      {
        StatusOUT3 = false;
      ...
      client.printf("OUT3=%d\r\n", StatusOUT3);
    }
    else if(strncmp(AnswerPackServer, "RESET", 5) == 0)
    {
      client.printf("#M#ESP32 RESET OK\r\n");
      Serial.println("RESET OK");
      esp_restart();
    }
    else if(strncmp(AnswerPackServer, "OUT?", 4) == 0)
    {
      client.printf("#M#OUT=%d%d%d%d\r\n",StatusOUT0, StatusOUT1, StatusOUT2, StatusOUT3);
    }
    else
    {
...
//С версии 9 и выше пакет  данных отправляется полный, а не сокрашенный.
//#D#date;time;lat1;lat2;lon1;lon2;speed;course;height;sats;hdop;inputs;outputs;adc;ibutton;params\r\n
  char SendSDPack[512] = "";//Сюда мы собираем данные в массив для отправки
  strcat(SendSDPack, "#D#");
  ...
  strcat(SendSDPack, kav);
  strcat(SendSDPack, hdop);
  strcat(SendSDPack, kav);
  strcat(SendSDPack, inputs);
  strcat(SendSDPack, kav);
  strcat(SendSDPack, outputs);
  strcat(SendSDPack, kav);
  strcat(SendSDPack, adc);
  strcat(SendSDPack, kav);
  strcat(SendSDPack, ibutton);
  strcat(SendSDPack, kav);
  strcat(SendSDPack, params);  
  ...
  if(strncmp(AnswerPack, TrueD, 5) == 0){
    AuthServer = true;
  }
  else{
    AuthServer = false;
