  ...
  else if(C.indexOf("SETOUT") >= 0)
  {
    ...
    if(i != -1)
    {
      ...
      switch(out)
      {
        case 0:
        {
          if(status == 0) output0 = false;
          if(status == 1) output0 = true;
          prefs.putBool("OUTPUT0", output0);
          SendMSG(SETOK);
          break;
        }
        case 1:
        {
          if(status == 0) output1 = false;
          if(status == 1) output1 = true;
          prefs.putBool("OUTPUT1", output1);
          SendMSG(SETOK);
          break;
        }
        case 2:
        {
          if(status == 0) output2 = false;
          if(status == 1) output2 = true;
          prefs.putBool("OUTPUT2", output2);
          SendMSG(SETOK);
          break;
        }
        case 3:
        {
          if(status == 0) output3 = false;
          if(status == 1) output3 = true;
          prefs.putBool("OUTPUT3", output3);
          SendMSG(SETOK);
          break;
        }
        default:
...
{
  //Serial.printf("Diagnostics:%s", S.c_str());
  switch(StatusCommand) 
  {
    ...
    {
      //StatusCommand = 0;
      if(S.indexOf("CONNECT OK") >= 0 || S.indexOf("ALREADY CONNECT") >= 0) //ALREADY CONNECT
      {
        CIPSTARTModem = true;
        StatusSendConn = true;
        StatusCommand = 0;
...
void loop()
{
  ...
  if(millis() - Timer3 > 10000)
  {
    ...
    StatusCommand = 0; //Не знаю на сколько  это повысит стабильность, но иногда  зависает  на  запросе времени
  }
